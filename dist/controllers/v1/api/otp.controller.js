...
class OtpController extends master_controller_1.default {
    constructor() {
        super();
    ...
    async fetchOtp(req, res) {
        const startMS = new Date().getTime();
        let resModel = { ...response_entity_1.ResponseEntity };
        let params;
        try {
            params = req.query;
            resModel = await this.otpModel.fetch(params);
            ...
            this.logger.error(JSON.stringify(resModel), `${this.constructor.name} : fetchOtp`);
        }
    }
}
