...
class FoodTypeModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM restraunts.food_type WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with food_type_id
            if (params.food_type_id) {
                query += `food_type_id = $${index} AND `;
                values.push(params.food_type_id);
                index += 1;
            }
            // filter with food_type
            if (params.food_type) {
                query += `food_type = $${index} AND `;
                values.push(params.food_type);
                index += 1;
            }
            // search filter
            if (params.search) {
                query += `(food_type LIKE $${index}) AND `;
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'foodTypeModel: fetch');
        }
        finally {
...
exports.FoodTypeModel = FoodTypeModel;
