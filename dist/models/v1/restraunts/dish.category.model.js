...
exports.DishCategoryModel = void 0;
...
class DishCategoryModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM restraunts.dish_cat WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with cat_name
            if (params.cat_name) {
                query += `cat_name = $${index} AND `;
                values.push(params.cat_name);
                index += 1;
            }
            // filter with dish_cat_id
            if (params.dish_cat_id) {
                query += `dm.dish_cat_id = $${index} AND `;
                values.push(params.dish_cat_id);
                index += 1;
            }
            // search filter
            if (params.search) {
                query += `(cat_name LIKE $${index}) AND `;
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'dishCategoryModel: fetch');
        }
        finally {
...
exports.DishCategoryModel = DishCategoryModel;
