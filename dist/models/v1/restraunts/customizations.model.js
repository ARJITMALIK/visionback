...
exports.CustomizationsModel = void 0;
...
class CustomizationsModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM restraunts.customizations WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with cust_id
            if (params.cust_id) {
                query += `cust_id = $${index} AND `;
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'customizationsModel: fetch');
        }
        finally {
...
exports.CustomizationsModel = CustomizationsModel;
