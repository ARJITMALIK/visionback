...
class VariantsModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM restraunts.variants WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with var_id
            if (params.var_id) {
                query += `var_id = $${index} AND `;
                values.push(params.var_id);
                index += 1;
            }
            // filter with name
            if (params.name) {
