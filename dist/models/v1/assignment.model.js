...
class AssignmentModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM election.assignment WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with assignment_id
            if (params.assignment_id) {
                query += `assignment_id = $${index} AND `;
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'partyModel: fetch');
        }
        finally {
