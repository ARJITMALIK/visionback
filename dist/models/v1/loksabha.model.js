...
class LoksabhaModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM election.loksabha_master WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with zone_id
            if (params.lok_id) {
                query += `lok_id = $${index} AND `;
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'loksabhaModel: fetch');
        }
        finally {
...
exports.LoksabhaModel = LoksabhaModel;
