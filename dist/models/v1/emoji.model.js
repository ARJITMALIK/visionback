...
class EmojiModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM election.emoji_master WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with zone_id
            if (params.emoji_id) {
                query += `emoji_id = $${index} AND `;
                values.push(params.emoji_id);
                index += 1;
            }
            if (params.status) {
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'emojiModel: fetch');
        }
        finally {
