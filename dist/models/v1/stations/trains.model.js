...
class TrainsModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = `SELECT * FROM trains.train_master WHERE `;
        const values = [];
        let index = 1;
        try {
            // filter with train_id
            if (params.train_id) {
                query += `train_id = $${index} AND `;
                values.push(params.train_id);
                index += 1;
            }
            // filter with train_name
            if (params.train_name) {
                query += `train_name = $${index} AND `;
                values.push(params.train_name);
                index += 1;
            }
            // filter with train_code
            if (params.train_code) {
                query += `train_code = $${index} AND `;
                values.push(params.train_code);
                index += 1;
            }
            // search filter
            if (params.search) {
                query += `(train_name LIKE $${index} OR train_code LIKE $${index}) AND `;
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'trainsModel: fetch');
        }
        finally {
...
exports.TrainsModel = TrainsModel;
