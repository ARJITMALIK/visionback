...
class ZonesModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM stations.zones WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with zone_id
            if (params.zone_id) {
                query += `zone_id = $${index} AND `;
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'zonesModel: fetch');
        }
        finally {
