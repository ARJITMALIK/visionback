...
class EmployeeModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM hr.employees WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with zone_id
            if (params.emp_id) {
                query += `emp_id = $${index} AND `;
                values.push(params.emp_id);
                index += 1;
            }
            if (params.role_id) {
                query += `role_id = $${index} AND `;
                values.push(params.dept_id);
                index += 1;
            }
            // filter with status
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'deliveryModel: fetch');
        }
        finally {
