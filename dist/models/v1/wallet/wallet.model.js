...
class WalletModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT p.*, w.opening_bal FROM payments.payments_master p LEFT JOIN payments.vendor_wallet_balance w ON w.outlet_id = p.outlet_id WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with zone_id
            if (params.pay_id) {
                query += `p.pay_id = $${index} AND `;
                values.push(params.pay_id);
                index += 1;
            }
            // filter with name
            if (params.order_id) {
                query += `p.order_id = $${index} AND `;
                values.push(params.order_id);
                index += 1;
            }
            if (params.outlet_id) {
                query += `p.outlet_id = $${index} AND `;
                values.push(params.outlet_id);
                index += 1;
            }
            // search filter
            if (params.search) {
                query += `(p.name LIKE $${index + 1})`;
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'WalletModel: fetch');
        }
        finally {
