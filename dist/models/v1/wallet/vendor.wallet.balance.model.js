...
exports.VendorWalletBalanceModel = void 0;
...
class VendorWalletBalanceModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM payments.vendor_wallet_balance WHERE ';
        const values = [];
        let index = 1;
        try {
            ...
            // filter with logged_at
            if (params.logged_at) {
                query += `logged_at = $${index} AND `;
                values.push(params.logged_at);
                index += 1;
            }
...
exports.VendorWalletBalanceModel = VendorWalletBalanceModel;
