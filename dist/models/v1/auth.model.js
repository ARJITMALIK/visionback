...
class AuthModel extends master_model_1.default {
    constructor() {
        super();
    }
    async fetch(params, limit = 1) {
        ...
        let query = 'SELECT * FROM auth.auth_master WHERE ';
        const values = [];
        let index = 1;
        try {
            // filter with zone_id
            ...
            this.logger.error(`DB Fetch Error: ${query} - Error: ${JSON.stringify(error)}`, 'AuthModel: fetch');
        }
        finally {
